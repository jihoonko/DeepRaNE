...
class FocalLoss(torch.nn.Module):
    ...
    def forward(self, input, target):
        ...
        loss = torch.nn.CrossEntropyLoss(reduction='none')(input, target)
        loss = loss * ((1. - logit[range(input.shape[0]), target]) ** self.gamma)
        return loss
...
def train(args):
    ...
    model.train()
    f_log = open("./logs/finetuning_focal.log", "a")
    while step_cnt < args.n_steps:
        ...
        for imgs, indices, rows, cols, vals, wheres in tqdm(iter(train_loader)):
            ...
            preds = _preds[indices, rows, cols]
            labels = torch.argmax(preds, dim=-1).to(args.default_device)
            
            loss = FocalLoss(gamma=2)(preds, vals).sum()
            ...
            if step_cnt % 1000 == 0:
                ...
                f_log.flush()
                model.eval()
                torch.save(model.module.state_dict(), f'./checkpoints/finetuning_focal_{step_cnt}.pkt')
                val_loss_sum, val_acc = 0., 0
                _TP1, _TN1, _FP1, _FN1 = 0, 0, 0, 0
                _TP2, _TN2, _FP2, _FN2 = 0, 0, 0, 0
                with torch.no_grad():
                    for i, (imgs, indices, rows, cols, vals, wheres) in enumerate(iter(val_loader)):
                        imgs = imgs.to(args.default_device)
                        indices, rows, cols, vals = indices.to(args.default_device), rows.to(args.default_device) - 381, cols.to(args.default_device) - 381, vals.to(args.default_device)
