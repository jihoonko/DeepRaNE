...
def train(args):
    ...
    model.train()
    f_log = open("./logs/finetuning_ce.log", "a")
    while step_cnt < args.n_steps:
        ...
        for imgs, indices, rows, cols, vals, wheres in tqdm(iter(train_loader)):
            ...
            loss = torch.nn.CrossEntropyLoss(reduction='sum')(preds, vals)
            loss.backward()
            optimizer.step()
            ...
            if step_cnt % 1000 == 0:
                ...
                f_log.flush()
                model.eval()
                torch.save(model.module.state_dict(), f'./checkpoints/finetuning_ce_{step_cnt}.pkt')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sampled-path", type=str, default='sampled/sampled.pkl',
